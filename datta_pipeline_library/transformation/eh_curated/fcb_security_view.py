...
fcb_view_name = dbutils.widgets.get(name="fcb_view_name")
process_status_view_name = dbutils.widgets.get(name="process_status_view_name")
process_status_table_name = dbutils.widgets.get(name="process_status_table_name")
fcb_star_join_table_name = dbutils.widgets.get(name="fcb_star_join_table_name")
...
df_schema_view = spark.sql(f"SHOW VIEWS IN `{uc_catalog_name}`.`{uc_curated_schema}`").select("viewName")
view_list = [row[0] for row in df_schema_view.select('viewName').collect()]
if fcb_view_name in view_list:
    spark.sql(f"""DROP VIEW `{uc_catalog_name}`.`{uc_curated_schema}`.{fcb_view_name}""")
    spark.sql(f"""CREATE OR REPLACE VIEW `{uc_catalog_name}`.`{uc_curated_schema}`.{fcb_view_name} 
              WITH SCHEMA EVOLUTION AS SELECT * FROM 
              `{uc_catalog_name}`.`{uc_curated_schema}`.{fcb_star_join_table_name} t 
              ...
              SELECT 1 FROM `{uc_catalog_name}`.`{security_schema}`.{security_function_name}('Company_Code') eu WHERE t.Company_Code = eu.allowed_value)
    ...
    assignViewPermission(uc_catalog_name,uc_curated_schema,fcb_view_name,tbl_owner_grp, security_end_user_aad_group_name, security_object_aad_group_name, security_functional_readers_aad_group, security_functional_dev_aad_group)
    
    print("FCB view dropped and recreated in the schema")
else: 
    spark.sql(f"""CREATE OR REPLACE VIEW `{uc_catalog_name}`.`{uc_curated_schema}`.{fcb_view_name} 
              WITH SCHEMA EVOLUTION AS SELECT * FROM 
              `{uc_catalog_name}`.`{uc_curated_schema}`.{fcb_star_join_table_name} t 
...
df_schema_view = spark.sql(f"SHOW VIEWS IN `{uc_catalog_name}`.`{uc_curated_schema}`").select("viewName")
view_list = [row[0] for row in df_schema_view.select('viewName').collect()]
if process_status_view_name in view_list:
    spark.sql(f"""DROP VIEW `{uc_catalog_name}`.`{uc_curated_schema}`.{process_status_view_name}""")
    spark.sql(f"""CREATE OR REPLACE VIEW `{uc_catalog_name}`.`{uc_curated_schema}`.{process_status_view_name} 
              WITH SCHEMA EVOLUTION AS SELECT * FROM `{uc_catalog_name}`.`{uc_raw_schema}`.{process_status_table_name}""")
    
    assignViewPermission(uc_catalog_name,uc_curated_schema,process_status_view_name,tbl_owner_grp, security_end_user_aad_group_name, security_object_aad_group_name, security_functional_readers_aad_group, security_functional_dev_aad_group)
        
    print("Process status view dropped and recreated in the schema")
else: 
    spark.sql(f"""CREATE OR REPLACE VIEW `{uc_catalog_name}`.`{uc_curated_schema}`.{process_status_view_name} 
              WITH SCHEMA EVOLUTION AS SELECT * FROM `{uc_catalog_name}`.`{uc_raw_schema}`.{process_status_table_name}""")
